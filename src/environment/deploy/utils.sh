...
function replace_versions() {
	sed -e "s|<appInsightsApiVersion>|${APP_INSIGHTS_API_VERSION}|" \
		...
		-e "s|<roleAssignmentsApiVersion>|${ROLE_ASSIGNMENTS_API_VERSIONS}|" \
		-e "s|<identitiesApiVersion>|${IDENTITIES_API_VERSIONS}|" \
		-e "s|<keyVaultsApiVersion>|${KEY_VAULTS_API_VERSIONS}|" \
		-e "s|<eventGridsApiVersion>|${EVENT_GRID_API_VERSIONS}|" \
		-e "s|<automationAccountsApiVersion>|${AUTOMATION_ACCOUNTS_API_VERSIONS}|" \
		-e "s|<batchAccountsApiVersion>|${BATCH_ACCOUNTS_API_VERSIONS}|" \
		-e "s|<disksApiVersion>|${DISKS_API_VERSIONS}|" \
		-e "s|<containerRegistriesApiVersion>|${CONTAINER_REGISTRIES_API_VERSIONS}|" \
		-e "s|<managedClustersApiVersion>|${MANAGED_CLUSTERS_API_VERSIONS}|" \
		$1 >$2
}
